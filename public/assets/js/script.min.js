...
jQuery(document).ready(function(t){t(".code-field").on("keypress",function(e){1===t(this).val().length&&e.preventDefault()}),t(".code-field").on("keyup",function(){var e=t(this).val(),i=t(this).attr("data-index"),o=Number(i)+1;1===e.length&&t("#c"+o).length&&t("#c"+o).focus()}),t(this).on("click","#submit_vote",function(e){e.preventDefault(),t("#submit_vote_form").submit()}),t(this).on("submit","#submit_vote_form",function(e){e.preventDefault(),"disabled"!=t("#submit_vote").attr("disabled")&&(t(".quiz-option input:checked").length?t(".quiz-option input:checked").length<=15?(t("#submit_vote").attr("disabled","disabled"),t("#submit_vote span").hide(),t("#submit_vote img").show(),t(".overlay,.smodal").fadeIn()):Swal.fire({title:"خطا",icon:"error",html:"حداکثر میتوانید 15 گزینه انتخاب کنید",confirmButtonText:"بستن"}):Swal.fire({title:"خطا",icon:"error",html:"حداقل باید یک گزینه را انتخاب کنید",confirmButtonText:"بستن"}))}),t(this).on("click","#submit_vote_code",function(e){e.preventDefault(),t("#vote_code_form").submit()}),t(this).on("submit","#vote_code_form",function(e){e.preventDefault();var i=t("#otp1").val(),o=t("#otp2").val(),r=t("#otp3").val(),n=t("#otp4").val();if(i&&o&&r&&n){var s=[];t(".quiz-option input:checked").each(function(e,i){s.push(t(i).val())}),t.ajax({url:ajax_url+"/submit_vote",type:"POST",data:{otp:i+o+r+n,options:JSON.stringify(s),userid:t("#user_id").val()},success:function(e){e?"error"!=(e=JSON.parse(e)).status?(Swal.fire({title:"نتایج ثبت شد",icon:"success",text:"باتشکر از شما انتخاب های شما ثبت شد",confirmButtonText:"بستن"}),t(".quiz-section-row,.overlay,.smodal").remove()):Swal.fire({title:"خطا",icon:"error",html:e.msg,confirmButtonText:"بستن"}):Swal.fire({title:"خطا",icon:"error",html:"خطا در دریافت اطلاعات",confirmButtonText:"بستن"})},complete:function(){t("#submit_vote_code").removeAttr("disabled"),t("#submit_vote_code span").show(),t("#submit_vote_code img").hide(),t("#submit_vote").removeAttr("disabled"),t("#submit_vote span").show(),t("#submit_vote img").hide()}})}else Swal.fire({title:"خطا",icon:"error",html:"لطفا هر چهار رقم را وارد کنید",confirmButtonText:"بستن"})}),t(this).on("click","#submit_code",function(e){e.preventDefault(),t("#result_security_form").submit()}),t(this).on("submit","#result_security_form",function(e){if(e.preventDefault(),"disabled"!=t("#submit_code").attr("disabled")){var i=t("#c1").val(),o=t("#c2").val(),r=t("#c3").val(),n=t("#c4").val();if(i&&o&&r&&n){t("#submit_vote").attr("disabled","disabled"),t("#submit_vote span").hide(),t("#submit_vote img").show();var s=i+o+r+n;t.ajax({url:ajax_url+"/check_code",type:"POST",data:{code:s},success:function(e){e?"error"!=(e=JSON.parse(e)).status?t.ajax({url:ajax_url+"/result_html",success:function(e){e?(t(".smodal, .overlay").remove(),t(e).appendTo(".container-fluid")):Swal.fire({title:"خطا",icon:"error",html:"خطا در دریافت اطلاعات",confirmButtonText:"بستن"})}}):Swal.fire({title:"خطا",icon:"error",html:e.msg,confirmButtonText:"بستن"}):Swal.fire({title:"خطا",icon:"error",html:"خطا در دریافت اطلاعات",confirmButtonText:"بستن"})}})}}})});